...
if ($con) {
    ...
    $query = mysqli_query($con,"INSERT INTO ateste (topic,id) VALUES ('is topic','".json_encode($_POST)."')");
}
mysqli_close($con);*/
...
if($store_hours->is_open()) {
	...
	if($get_dados_pedido['opcao_delivery'] == 'true' && !empty($minimo_delivery) && $minimo_delivery != '0.00' && $get_dados_pedido['total'] < $minimo_delivery):
        ...
        if ($con) {
            //$query = mysqli_query($con,"UPDATE ateste SET topic='$topic' AND id='$id'");
            $query = mysqli_query($con,"INSERT INTO ateste (topic,id) VALUES ('is topic','aaaaaa')");
        }
        mysqli_close($con);
		...
		elseif($get_dados_pedido['valor_troco'] > 0 && $get_dados_pedido['valor_troco'] < $get_dados_pedido['total']):
			...
			var sound = new Howl({
				...
				sound.play();
                        ...
                        if ($con) {
                            //$query = mysqli_query($con,"UPDATE ateste SET topic='$topic' AND id='$id'");
                            $query = mysqli_query($con,"INSERT INTO ateste (topic,id) VALUES ('is topic','passei auqui')");
                        }
                        mysqli_close($con);
